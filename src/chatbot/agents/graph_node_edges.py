...
def _sanitize_ai_message(message: AIMessage) -> AIMessage:
    """Strip non-serializable metadata from Gemini AIMessages."""
    return AIMessage(
...
class State(TypedDict):
    ...
    #     BaseMessage
    # ]  # to exclude all system messages and messages generated by the agent (passed on graph called contained conversation summary as human message).  # list ai and human messages
    ...
    # ---- Per-request references (formerly on the singleton) ----
...
class GraphNodesMixin:
    ...
    def judge_node(self, state: State) -> Dict:
        ...
        if score.judgement_binary.lower() == "no":
            ...
            return {
                "messages": [HumanMessage(content=translate_prompt()["use_tool_msg"])],
    ...
    def generate_helper(self, state, system_message_generate):
        # Message history passed on graph._graph.asstream(), contains conversatio summary
        ...
        if messages_history:
            ...
            llm_messages = [system_message_generate] + _query_message
